...
import com.smart.enums.RandomType;
...
public class RandomComponentSettingsDialog extends CommonDialog {
    ...
    public RandomComponentSettingsDialog(String componentId, String cacheComponentId, Project project, JPanel canvasPanel, String type, VirtualFile file) {
        super(false);
    ...
    public JPanel createConfigurationPanel() {
        ...
        JLabel descriptionLabel = new JLabel("随机数生成,注意生成的随机值为字符串");
        ...
        // 添加操作类型行
        tableModel.addRow(new Object[]{"随机类型", RandomType.NUMBER});
        tableModel.addRow(new Object[]{"长度", "6"}); // 默认长度6
        ...
        valueColumn.setCellEditor(new DefaultCellEditor(new JComboBox<>()) {
            private ComboBoxCellEditor randomTypeEditor;
            private DefaultCellEditor textFieldEditor = new DefaultCellEditor(new JTextField());

            {
                randomTypeEditor = new ComboBoxCellEditor(RandomType.values(), paramsTable, 0);
            }
            ...
            public Component getTableCellEditorComponent(JTable table, Object value,
                ...
                if (row == 0) { // 随机类型行
                    return randomTypeEditor.getTableCellEditorComponent(
            ...
            public Object getCellEditorValue() {
                if (paramsTable.getEditingRow() == 0) {
                    return randomTypeEditor.getCellEditorValue();
                }
        ...
        DefaultTableCellRenderer customRenderer = new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable table, Object value,
                ...
                if (row == 0 && value instanceof RandomType) {
                    setText(((RandomType) value).getDisplayName());
                }
        ...
        public ComboBoxCellEditor(Object[] items, JTable table, int row) {
            ...
            comboBox.setRenderer(new DefaultListCellRenderer() {
                ...
                public Component getListCellRendererComponent(JList<?> list, Object value,
                    ...
                    if (value instanceof RandomType) {
                        setText(((RandomType) value).getDisplayName());
                    }
                    return this;
                }
    ...
    public void loadExistingSettings() {
        ...
        if (componentInfo != null && componentInfo.getComponentProp() != null) {
            ...
            if (paramProps != null && !paramProps.isEmpty()) {
                ...
                // 设置随机类型
                try {
                    model.setValueAt(RandomType.valueOf(paramProps.get(0).getVal()), 0, 1);
    ...
    private boolean validateInput() {
        ...
        String length = (String) model.getValueAt(1, 1);
        String bindValue = (String) model.getValueAt(2, 1);
        
        if (length == null || length.trim().isEmpty()) {
            Messages.showWarningDialog("请输入长度", "验证失败");
            return false;
        }
        ...
        try {
            ...
            if (len <= 0) {
                Messages.showWarningDialog("长度必须大于0", "验证失败");
                return false;
            }
        } catch (NumberFormatException e) {
            Messages.showWarningDialog("请输入有效的长度数值", "验证失败");
            return false;
        }
    ...
    public void saveSettings() {
        ...
        typeProp.setSeq("1");
        typeProp.setVal(((RandomType) model.getValueAt(0, 1)).name());
        typeProp.setDataType(DataType.STRING);
        props.add(typeProp);

        // 保存长度
        ParamProp lengthProp = new ParamProp();
        lengthProp.setSeq("2");
        lengthProp.setVal((String) model.getValueAt(1, 1));
        lengthProp.setDataType(DataType.STRING);
        props.add(lengthProp);
